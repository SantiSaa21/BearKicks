...
private class FakeCartRepo : ICartRepository {
    ...
    override fun observeOrders(uid: String) = kotlinx.coroutines.flow.flow<List<com.bearkicks.application.features.cart.data.database.entity.OrderEntity>> { emit(emptyList()) }
    override fun observeOrderItems(orderId: String) = kotlinx.coroutines.flow.flow<List<com.bearkicks.application.features.cart.data.database.entity.OrderItemEntity>> { emit(emptyList()) }
